...
def extract_mobile_number(text):
    ...
    # Found this complicated regex on : https://zapier.com/blog/extract-links-email-phone-regex/
    # phone = re.findall(re.compile(r'(?:(?:\+?([1-9]|[0-9][0-9]|[0-9][0-9][0-9])\s*(?:[.-]\s*)?)?(?:\(\s*([2-9]1[02-9]|[2-9][02-8]1|[2-9][02-8][02-9])\s*\)|([0-9][1-9]|[0-9]1[02-9]|[2-9][02-8]1|[2-9][02-8][02-9]))\s*(?:[.-]\s*)?)?([2-9]1[02-9]|[2-9][02-9]1|[2-9][02-9]{2})\s*(?:[.-]\s*)?([0-9]{4})(?:\s*(?:#|x\.?|ext\.?|extension)\s*(\d+))?'), text)
    mob_num_regex = r'(\+212|0)?[ .-]?(6|7)[ .-]?(\d{2})[ .-]?(\d{2})[ .-]?(\d{2})[ .-]?(\d{2})'
    phone_matches = re.findall(mob_num_regex, text)
    for match in phone_matches:
        # Join the matched parts and remove any spaces, dots, or dashes
        number = ''.join(match).replace(' ', '').replace('.', '').replace('-', '')

        # Validate number length based on the prefix
